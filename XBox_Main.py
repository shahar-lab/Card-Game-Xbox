...
dataFile.write("subject, block, condition, trial, offer_right, offer_left, exp_value_right, exp_value_left, key, key_ID, chosen_card, unchosen_card, rt, reward\n")
...
def main():
    ...
    if trainPhase:
        ...
        while True:
            ...
            if (events.type == pygame.JOYBUTTONDOWN):
                ...
                if events.button == 0:
                    training2 = visual.ImageStim(win, image="training2.jpg",  units='norm', size=[2,2], interpolate = True)
                    training2.draw()
                    win.update()
                    break
        while True:
    ...
    if gamePhase:
        ...
        for x in possibleOrder[indx]:
            if (x == 0):
                blockCnt = blockCnt + 1
                startBlock = "startblock" + str(blockCnt) + ".jpg"
                ...
                while True:
                    ...
                    #Event 0 -> Pressed A Button
                        if events.button == 0:
                            rumbleCond = "press"
                            mainExperimentModes(dataFile, blockCnt, subjectN, win, rumbleCond)
                            break
                end.draw()
                win.update()
...
def trainFunc():
    ...
    # Starting trials in block
    for t in range(1, 6):
        # Draw the stimuli and update the window
        img = "train" + str(t) + ".jpg"
        tTrial = visual.ImageStim(win, image=img,  units='norm', size=[2,2], interpolate = True)
        tTrial.draw()
        win.update()
        leftB = "train" + str(t) + "left" + ".jpg"
        rightB = "train" + str(t) + "right" + ".jpg"
        leftchoice = visual.ImageStim(win, image=leftB,  units='norm', size=[2,2], interpolate = True)
        rightchoice = visual.ImageStim(win, image=rightB,  units='norm', size=[2,2], interpolate = True)
        # While loop is True if the program is waiting for a response
        # While loop waits for a reponse and writes data in dataFile
        while True:
            events = pygame.event.poll()
            if (events.type == pygame.JOYBUTTONDOWN):
                ...
                if events.button == 4:
                    ...
                    dataFile.write("%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s," 
                        % (
                            ...
                            stimapr,
                            key,
                            "card number " + str(stim_id),
                            "card number" + str(other_id),
                            RT,
                        )
                    )
                    if (rumbleCond == "press"):
